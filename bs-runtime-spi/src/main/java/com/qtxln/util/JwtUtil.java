...
public class JwtUtil {
     ...
     * JWT 生成密钥使用的密码
     */
    private static final String JWT_RULE = "qtxln";
    ...
    public static String buildJWT(SignatureAlgorithm alg, Key key, String sub, String aud, String jti, String iss, Date nbf, Integer duration) {
        // jwt的签发时间
        Date iat = DateUtil.getDate();
        // jwt的过期时间，这个过期时间必须要大于签发时间
        Date exp = null;
        if (duration != null) {
            exp = DateUtil.getDate(duration);
        }
        // 获取JWT字符串
    ...
    public static String buildJWT(String sub) {
        return buildJWT(sub, null, UUID.randomUUID().toString(), null, null, 24*60*60);
    }
    ...
    public static void main(String[] args) {
        ...
        map.put("userId", "1");
        map.put("roleId", "2");
        String s = buildJWT(map.toString());
        System.out.println(s);
        Jws<Claims> jws = parseJWT(generateKey(JWT_ALG, JWT_RULE), s);
